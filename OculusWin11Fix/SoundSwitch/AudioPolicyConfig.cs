...
namespace SoundSwitch.Audio.Manager.Interop.Client.Extended {
  public sealed class AudioPolicyConfig : IAudioPolicyConfig {
    ...
    public static extern void DllGetActivationFactory([In][MarshalAs(UnmanagedType.HString)] string iid, [Out] out IntPtr factory);
    ...
    [UnmanagedFunctionPointer(CallingConvention.StdCall)]
    private delegate HResult GetPersistedDefaultAudioEndpointDelegate(IntPtr audioPolicyConfigFactory, uint processId, EDataFlow flow, ERole role, [MarshalAs(UnmanagedType.HString)] ref string deviceId);

    [UnmanagedFunctionPointer(CallingConvention.StdCall)]
    private delegate HResult SetPersistedDefaultAudioEndpointDelegate(IntPtr audioPolicyConfigFactory, uint processId, EDataFlow flow, ERole role, [MarshalAs(UnmanagedType.HString)] string deviceId);
    ...
    public AudioPolicyConfig() {
      ...
      DllGetActivationFactory("Windows.Media.Internal.AudioPolicyConfig", out _factory);
    ...
    public void SetPersistedDefaultAudioEndpoint(uint processId, EDataFlow flow, ERole role, string deviceId) {
      ...
      if (result != HResult.Code.S_OK && result != processNoAudio)
